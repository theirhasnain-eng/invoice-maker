...
export default function Inventory() {
  ...
  const fetchItems = async () => {
    try {
      // Centralized api instance automatically injects JWT header
  ...
  const handleOpenCreateModal = () => {
    setEditingId(null);
    setFormData({ name: '', category: '', price: '', quantity: '', minQuantity: 5 });
    setIsModalOpen(true);
  };
                  ...
                  filteredItems.map((item) => {
                    ...
                    return (
                      <tr key={item._id} className="hover:bg-gray-50">
                        ...
                        <td className="p-4 text-right font-medium text-gray-900">${item.price.toFixed(2)}</td>
